...
BOT_TOKEN = '8626089348:AAGVUIUWGl9ktWdusDZH8yarY2QcNH0Yzlw'
...
user_client = TelegramClient('incomepro_user_session', API_ID, API_HASH)
...
# Temporarily hold a message while waiting for its custom time
pending_posts = {}

schedule_data = {
    "morning_msgs": [], # Each item: {"msg": event.message, "time": "08:15"}
    "evening_msgs": []  # Each item: {"msg": event.message, "time": "20:30"}
}
...
def update_job_schedule():
    ...
    try:
        # Schedule each morning post at its specific individual time
        ...
        # Schedule each evening post at its specific individual time
...
async def auto_repost_handler(event):
    if event.photo or event.media:
        try:
            ...
            keywords = ["congrat", "congrats", "congratulation", "congratulations", "🎉", "🥳", "credit alert", "credit", "more winnings", "successful", "withdrawal"]
...
async def main():
    ...
    if not await user_client.is_user_authorized():
        print("❌ Error: User session is not authorized.")
        return
